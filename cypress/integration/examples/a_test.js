...
context('Actions', () => {
  beforeEach(() => {
    ...
    console.log("Visiting Main Site - automationpractice.com")
...
// Zone Digital Test
    ...
    cy.title().should('eq', 'My Store')   // Check that title of page is correct with an assertion
    console.log("Checking that title should equal My Store")

    cy.get('[data-id-product="2"]').eq( 0 ).click()   // Check that the product id is 2 for right product (blouse
    console.log("Checking that title should equal My Store")
...
// end of News Bits
   ...
   cy.get( "button[class^=\"btn btn-default button button-medium exclusive\"]" ).click().wait( 4000 )  // Clock on create new account submit button , wait for 1000 for page to load (just in case) not usually requiered  
